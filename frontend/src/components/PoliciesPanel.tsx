...
export function PoliciesPanel({ isOpen, onClose }: PoliciesPanelProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="relative w-full max-w-5xl h-[85vh] m-4 flex overflow-hidden rounded-2xl bg-slate-900 border border-slate-700/50 shadow-2xl">
                {/* Policy Tabs - Left Sidebar */}
                <div className="w-64 border-r border-slate-700/50 flex flex-col bg-slate-800/50">
                    <div className="p-4 border-b border-slate-700/50">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400">Dokumen Perusahaan</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2">
                        {loading ? (
                            ...
                            policies.map((policy) => (
                                ...
                                >
                                    <span className={selectedPolicy?.id === policy.id ? 'text-primary-400' : 'text-slate-500'}>
                                        {getIcon(policy.icon)}
                                    </span>
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="h-14 px-6 flex items-center justify-between border-b border-slate-700/50 bg-slate-800/30">
                    ...
                    <div className="flex-1 overflow-y-auto p-6">
                        {selectedPolicy ? (
                            <div className="prose prose-invert prose-slate max-w-none 
                                ...
                                prose-h1:text-2xl prose-h1:mb-6 prose-h1:pb-3 prose-h1:border-b prose-h1:border-slate-700
                                prose-h2:text-xl prose-h2:mt-8 prose-h2:mb-4 prose-h2:text-primary-400
                                prose-h3:text-lg prose-h3:mt-6 prose-h3:mb-3
                                prose-p:text-slate-300 prose-p:leading-relaxed
                                prose-li:text-slate-300 prose-li:marker:text-primary-500
                                prose-strong:text-white prose-strong:font-semibold
                                prose-table:border-collapse prose-table:w-full
                                prose-th:bg-slate-800 prose-th:text-white prose-th:font-medium prose-th:px-4 prose-th:py-2 prose-th:border prose-th:border-slate-700
